...
def seed(conn: sqlite3.Connection, *, rnd: random.Random | None = None) -> str:
    ...
    for i, vm in enumerate(VMS):
        ...
        conn.execute("INSERT OR REPLACE INTO fdb (device, interface, mac, source) "
                     "VALUES ('core1', ?, ?, 'demo')", (port, mac_of[vm]))
    for host, _ip in BASEMENT:
        conn.execute("INSERT OR REPLACE INTO fdb (device, interface, mac, source) "
                     "VALUES ('edge1', 'e1/0/8', ?, 'demo')", (mk_mac(host),))
    conn.execute("INSERT OR REPLACE INTO fdb (device, interface, mac, source) "
                 "VALUES ('edge1', 'e1/0/10', ?, 'demo')", (mk_mac("nas1"),))
