...
### M0 — Bootstrap (2026-08-22, Beat Farmer)

```
Milestone: M0 Bootstrap        Dates: 2026-08-22 (one session)
Briefs: 5 Sonnet (audio-dsp, economy-sim, dev-panel, core-coverage, audio-integration) + 1 QA pass;
        seed (docs, contracts, vertical slice, harness) Fable-authored.
Retries: none reported at brief level (all 5 STATUS: DONE first time; internal retries inside briefs were
         ≤ 1 and self-resolved).   Escalations: Sonnet→Opus 0, Opus→Fable 0, Fable→Austin 2 (stack options;
         re-asked with visual/taste criteria).
What the ladder caught: nothing needed to climb — briefs were crisp enough that "documented-not-fixed" beat
         escalation three times (RNG fork order-dependence, stage-7/8 density plateau, skill-floor at 0.6 acc).
What slipped past the gates (found by Austin): pending playtest.
Token hot-spots (subagent tokens): dev-panel ≈ 253 k (NuGet packaging quirk root-caused via project.assets.json),
         audio-dsp ≈ 197 k, audio-integration ≈ 191 k, core-coverage ≈ 170 k, economy-sim ≈ 146 k
         (≈ 0.96 M total for ~5 k lines of tested code + evidence); wall ≈ 14–25 min each, run in parallel.
Playbook changes: §14 learnings below.   CLAUDE.md changes: commands/layout refreshed by PM after merges.
```

### M1 — Feel (2026-08-22, Beat Farmer)

```
Milestone: M1 Feel             Dates: 2026-08-22 (same session as M0, after PR #1 opened)
Briefs: 6 Sonnet (art-direction-candidates, calibration-screen, music-arrangement, judgment-feel-and-juice,
        m1-polish, m1-hygiene) + 2 QA passes (wave-1 QA, final QA); PM-authored: briefs, merges, CLAUDE.md, review page.
Retries: none at brief level (6/6 STATUS: DONE first time).   Escalations: Sonnet→Opus 0, Opus→Fable 0,
        Fable→Austin 1 (look decision — presented as an in-engine comparison page, pending).
What the ladder caught: nothing climbed; findings flowed PM→brief instead (QA → m1-polish; juice → frame cap).
What slipped past the gates: vsync not honored on the dev box (~3000 fps) — caught by the juice brief's own Perf
        readout, not by a gate → harness now records nothing about pacing because it is uncapped by design; the
        real-run `[game] … cap=` log line + Perf readout are the evidence.
Token hot-spots (subagent tokens): juice ≈ 252 k, calibration ≈ 222 k, music ≈ 218 k, art-direction ≈ 197 k,
        polish ≈ 191 k, hygiene ≈ 135 k, QA ≈ 93 k (+ final QA) ≈ 1.3 M total; wall 15–24 min each, waves in parallel.
Playbook changes: learnings 9–12 below.   CLAUDE.md changes: PM applied all agent-proposed doc lines at merge time (0 conflicts).
```

### M2 — Defend (2026-08-22 → 23, Beat Farmer)

```
Milestone: M2 Defend (pivot to rhythm tower defense)     Dates: 2026-08-22 (evening) → 2026-08-23 (four PM sessions)
Briefs: 1 Opus seed (m2-defend-seed: ADR-0003 + GDD v0.2 + DefenseSession slice + harness evidence) + 11 Sonnet
        (playtest-round-2, voices-roster, charge-audio, defense-bot-and-sim, pest-roster-and-waves, build-phase-shop,
        groove-overcharge-and-harvest, procedural-art-pass [+ Opus design review], retire-osu-field, voice-unlocks, defense-juice)
        + 1 QA pass; PM-authored code: playtest-loop fixes (routing, stacked primitives, decay, dry turrets, audio queue,
        Kick cost 0→12, WaveGenerator plugged into the scene), all merges, docs (GDD sync, handoff, test guide, this retro).
Retries: none at brief level (every brief STATUS: DONE; two were interrupted by the session/user and *continued from WIP*).
Escalations: Sonnet→Opus 0 formal (+ 1 PM-requested architect design review on the art pass — it caught a partial-override
        bug); Opus→Fable 0; Fable→Austin 4 (pivot options 1/2/3 → picks 1-A/2-A/3-A→C; balance hold + "voices are bought,
        not given" → Decision 4).
What the ladder caught: the architect review (Windmill collapsing under a partial theme override); the PM integration
        gates caught free-Kick stacking (a rule no brief owned) and the sim-vs-real-run mismatch (sim tuned on a
        pre-placed six-turret field while the player builds from a shop — wave 2b realigned the simulator with CreateRun).
What slipped past the gates (found by Austin): round-1/2 playtest — turret routing, stacked primitives, charge decay too
        fast, dry turrets silent, second-monitor audio glitching (MonoGame InactiveSleepTime ≥ audio queue); **round 4 (after
        the PR hand-over): a player-started wave never got its song — the defense was silent for humans since the shop brief**
        (bot path ≠ player path; offline WAV ≠ live device — learning 20); pending: feel playtest.
Token hot-spots (subagent tokens): harvest ≈ 569 k (largest scope: Groove×Sap + tiers + Harvest/Greenhouse + sim rewire + retune),
        seed ≈ 433 k (Opus), pests ≈ 358 k, bot-sim ≈ 342 k, retire ≈ 342 k, unlocks ≈ 338 k, voices ≈ 333 k, charge-audio ≈ 313 k,
        juice ≈ 290 k, shop ≈ ?+159 k (interrupted + continued), playtest-2 ≈ 174 k, QA ≈ 129 k, art = unknown (stopped before its
        report) ⇒ ≈ 4 M total; wall 9–59 min each (median ≈ 30); waves 1 / 2a ran in parallel (disjoint scopes), 2b → 2c → 2d
        sequential (shared DefenseScene / BalanceConfig / DevPanel). Session 4 alone (retire → unlocks → juice → QA, all sequential,
        PM docs in parallel) ≈ 1.1 M subagent tokens in ≈ 2 h wall.
Gate results at hand-over (1bf7bdb): 198 Core + 42 Audio, 0 warnings; legacy 134/15/0; bot 9000 f prestiges 1; 14000 f prestiges 2;
        FullyGrown via --bot 1 --wave 8; CI command PASSED; four themes PASSED + read; sim survival p50 7 / max 8 ≤ 12 (balance hold).
Playbook changes: learnings 13–19 below.   CLAUDE.md changes: PM applied every agent-proposed doc delta at merge time;
        pivot callouts rewritten at the retirement; commands/layout refreshed twice (seed, retirement).
```
