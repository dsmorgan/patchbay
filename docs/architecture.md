...
| 5.5 ✅ | firewall config history: the opnsense collector pulls config.xml over the API (backup page privilege on the existing key), redacts secret-bearing elements to content hashes, strips revision-block noise, and keeps revisions in `config_revisions`; /configs merges them with the Oxidized timeline. Snapshots embed the latest revision, scrubbed a second time on the way in. Oxidized-over-SSH remains a documented fallback | a firewall rule change shows up in /configs like any switch change |
